var animation = (function (exports) {
  ...
  Rules.is.number = {
    ...
    test: (value)=>{ return !isNaN(value); }
  };

  for (let type in Rules) {
    for(let name in Rules[type]){
      Rules[type][name] = (value)=>{ return test(Rules[type[name],value]) };
    }
  }
  ...
  function Point (x, y) {
    let test = Rules.is.number(x);
    if (test.passed) { throw 'The paremeter x must be an integer' }
    ...
    [{
      ...
      descriptor: {
        ...
        value: y,
        get: ()=>{ return y },
        set: (value)=>{let test = Rules.is.number(value); if(test.passed){ y = value; }else{ throw test.error() } },
      }
    },
    {
      property: 'x',
      descriptor: {
        ...
        value: x,
        get: ()=>{ return x },
        set: (value)=>{let test = Rules.is.number(value); if(test.passed){ x = value; }else{ throw test.error() } },
      }
